...
import { StaticSovereignRouter } from "@aristotle/mesh-runtime";
...
const anchorSpec = process.env.MESH_TRUST_ANCHORS ??
    `${localMaeId}:127.0.0.1:7004`;
const trustAnchors = anchorSpec
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean)
    .map((spec) => {
...
app.post("/v1/mesh/route", (req, res) => {
    ...
    if (!mae_id)
        return res.status(400).json({ ok: false, error: "missing_mae_id" });
