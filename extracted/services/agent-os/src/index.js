...
const serviceOrigin = (serviceName, hostEnvKey, portValue) => {
...
const agents = new Map(defaultAgents.map((agent) => [agent.id, agent]));
...
const fingerprint = (namespace, seed) => `${namespace}-${seed.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "unknown"}`;
const agentIdentityContext = (agentId) => {
...
const workspaceIdentityContext = (workspaceId) => {
    if (!workspaceId)
        return {};
...
const policyCompileCache = new Map();
const envelopeValidationCache = new Map();
const policyCompileInflight = new Map();
const envelopeValidationInflight = new Map();
let killSwitchSnapshot = { state: "inactive", checkedAt: 0 };
let meshTelemetrySnapshot = { degradedNodes: [], checkedAt: 0 };
let persistQueue = Promise.resolve();
let autonomyLoopRunning = false;
const createMissionSteps = (requiredTools) => [
    {
        id: id("step"),
...
const ensureMissionMemory = (missionId) => {
...
const commitLedgerEvent = async (missionId, eventKind, payload) => {
    try {
...
const missionTasks = (missionId) => [...executionTasks.values()]
    .filter((task) => task.missionId === missionId)
    .sort((left, right) => left.createdAt.localeCompare(right.createdAt));
const missionToolLeases = (missionId) => [...toolLeases.values()].filter((lease) => lease.missionId === missionId && lease.state === "leased");
const findMissionWorkspace = (missionId) => [...workspaces.values()].find((workspace) => workspace.missionId === missionId);
const authorityMatchesTargetSystem = (authority, targetSystem) => {
    const normalizedAuthority = authority.toLowerCase();
    const normalizedTarget = targetSystem.toLowerCase();
    if (normalizedAuthority === normalizedTarget)
        return true;
    if (normalizedAuthority.startsWith(`${normalizedTarget}.`))
        return true;
    if (normalizedAuthority.includes(normalizedTarget))
        return true;
    if (normalizedTarget === "ledger" && /ledger|evidence/.test(normalizedAuthority))
        return true;
    return false;
};
const selectMissionAuthorityAnchor = (mission) => mission.requiredAuthorities.find((authority) => authorityMatchesTargetSystem(authority, mission.targetSystem)) ??
    mission.requiredAuthorities[0] ??
    "mission.command";
const isoAfter = (offsetMs) => new Date(Date.now() + offsetMs).toISOString();
const parseTimestamp = (value) => {
    if (!value)
        return Number.NaN;
    return new Date(value).getTime();
};
const buildTaskExecutionContext = (mission, title, ownerRole, requiredTools) => {
    const workspace = findMissionWorkspace(mission.id);
    const commandHints = ownerRole === "auditor"
...
const buildMissionPolicyText = (mission) => [
...
const compilePolicyArtifact = async (policyName, policyText) => {
...
const validateEnvelopeArtifact = async (payload) => {
...
const readKillSwitchState = async () => {
    ...
    try {
        const [kernelHealth, gateHealth] = await Promise.all([
            fetch(`${governanceKernelBase}/health`).then((response) => response.json()),
            fetch(`${executionGateBase}/health`).then((response) => response.json())
        ]);
        const nextState = kernelHealth.killSwitchState === "active" || gateHealth.killSwitchState === "active" ? "active" : "inactive";
...
const prewarmTaskGovernanceArtifacts = (mission, task, phase = "dispatch") => {
...
const prewarmToolActionExecutionArtifacts = (mission, task, action) => {
...
const setAgentRuntimeStatus = (agentId, status) => {
    const agent = agents.get(agentId);
    if (!agent)
        return;
    agents.set(agentId, { ...agent, status, lastHeartbeat: now() });
};
const countsTowardAgentCapacity = (task) => {
...
const agentTaskLoad = (agentId) => [...executionTasks.values()].filter((task) => task.assignedAgentId === agentId && countsTowardAgentCapacity(task)).length;
const allocateAgent = (mission, fallbackRole = "executor") => {
    const rankAgents = (pool) => [...pool].sort((left, right) => {
    ...
    const preferredRoleMatch = rankAgents(roleMatchedMissionCandidates)[0] ??
        rankAgents(roleMatchedGlobalCandidates)[0];
    if (preferredRoleMatch)
        return preferredRoleMatch;
...
const createExecutionTask = (mission, title, ownerRole, requiredTools, input, coordination) => {
    const timestamp = now();
...
const createExecutionReceipt = (missionId, taskId, agentId, summary, outcome, evidenceRefs, governanceRefs = []) => {
    const receipt = {
        id: id("receipt"),
        missionId,
        taskId,
...
const hasReceiptForTask = (taskId, outcome) => [...executionReceipts.values()].some((receipt) => receipt.taskId === taskId && (outcome ? receipt.outcome === outcome : true));
const taskDependencyState = (task) => {
...
const updateTaskGovernance = (taskId, governance) => {
...
const normalizeGovernanceDecision = (governance) => ({
...
const isPotentiallyDestructivePayload = (payload) => {
    ...
    return (/\b(rm|del|erase|rmdir|rd|delete|drop|truncate|overwrite)\b/.test(payloadText) ||
        /\bremove-item\b/.test(commandText) ||
        /\bcheckout\s+--\b/.test(commandText) ||
        /\breset\s+--hard\b/.test(commandText));
};
const deriveDegradedNodes = (mission, task, action) => {
...
const resolveAuthorityRoute = async (mission, options) => {
...
const claimTaskForAgent = (task, agentId, statusNote) => {
    const timestamp = now();
    const claimedTask = {
        ...task,
        status: "running",
...
const updateTaskHeartbeat = (task, agentId, statusNote) => {
    const timestamp = now();
    const updatedTask = {
        ...task,
        updatedAt: timestamp,
...
const taskToolActions = (taskId) => [...toolActions.values()]
    .filter((action) => action.taskId === taskId)
    .sort((left, right) => left.createdAt.localeCompare(right.createdAt));
const renewMissionLeases = async (missionId, actor, reason, renewalWindowMs = leaseRenewalWindowMs, extensionMs = leaseExtensionMs) => {
    const timestamp = now();
    const renewed = [];
    for (const lease of [...toolLeases.values()]) {
        if (lease.missionId !== missionId || lease.state !== "leased")
            continue;
...
const queueTaskRetry = async (task, actor, reason) => {
    const mission = missions.get(task.missionId);
    if (!mission)
    ...
    if (attempts >= taskMaxAttempts) {
        ...
        return { ok: false, error: "retry_exhausted", task: blockedTask };
    }
    const queuedTask = {
        ...task,
        status: "queued",
        ...
        output: {
            ...(task.output ?? {}),
            retryReason: reason,
            continuityRecovery: task.governance?.route?.continuity === "degraded" || task.governance?.route?.continuity === "disconnected"
                ? {
    ...
    return { ok: true, task: queuedTask };
};
const assessToolActionProposal = (mission, task, toolId, payload) => {
    ...
    const sensitiveConstraints = matchingLease?.constraints.filter((constraint) => /destructive|manual approval|required/i.test(constraint)) ?? [];
...
const assessToolActionExecutionGovernance = async (mission, task, action) => {
    const evaluatedAt = now();
    const reasons = [];
    ...
    if (!assignedAgent)
        reasons.push("Tool action agent is unavailable.");
    if (!task.requiredTools.includes(action.toolId))
        reasons.push(`Task is not authorized to use tool ${action.toolId}.`);
    if (!matchingLease)
        reasons.push(`Mission does not hold an active lease for tool ${action.toolId}.`);
    const sensitiveConstraints = matchingLease?.constraints.filter((constraint) => /destructive|manual approval|required/i.test(constraint)) ?? [];
    if (isPotentiallyDestructive && sensitiveConstraints.length > 0) {
        reasons.push(`Tool action requires operator approval: ${sensitiveConstraints.join("; ")}.`);
    }
    ...
    if (reasons.length === 0 && envelopeId) {
        try {
            ...
            const admissibility = (await admissibilityResponse.json());
    ...
    if (reasons.length === 0 && envelopeId && warrantId) {
        try {
            ...
            const commitDecision = (await commitPointResponse.json());
...
const assessTaskGovernance = async (mission, task, phase) => {
    const evaluatedAt = now();
    const reasons = [];
    const killSwitchState = await readKillSwitchState();
    const assignedAgent = agents.get(task.assignedAgentId);
    const activeLeases = missionToolLeases(mission.id);
    const missingLeaseTools = task.requiredTools.filter((toolId) => !activeLeases.some((lease) => lease.toolId === toolId));
    if (killSwitchState === "active") {
        reasons.push("Kill switch active.");
    }
    if (!assignedAgent) {
...
const blockTaskForGovernance = async (mission, task, governance, phase) => {
    ...
    createExecutionReceipt(mission.id, task.id, task.assignedAgentId, `${task.title} blocked by governance review.`, "blocked", [task.id, mission.id], [
...
const finalizeGovernedCompletion = async (mission, task, governance) => {
    ...
    if (witnessRequired) {
        try {
            ...
            witnessReceipt = (await witnessResponse.json());
    ...
    try {
        ...
        const finalityCertificate = {
            ...
            decisionId: decision.id,
            warrantId: governance.warrantId,
            receiptIds: [witnessReceipt?.id].filter((value) => Boolean(value)),
            ledgerCommitIndex: -1,
        ...
        }).then((response) => response.json()).catch(() => ({ index: undefined }));
...
const dispatchNextEligibleTask = async (mission, dispatchReason = "Dispatched by agent-os execution loop.", selector) => {
    const timestamp = now();
    ...
    const claimedTask = claimTaskForAgent({
        ...nextQueued,
        governance
    }, nextQueued.assignedAgentId, dispatchReason);
...
const completeTaskWithGovernance = async (mission, task, options) => {
    const timestamp = now();
    const queuedTaskReleaseState = new Map(missionTasks(mission.id)
    ...
    const finalizedGovernance = completionGovernance.status === "approved"
    ...
    if (finalizedGovernance.status === "blocked") {
        ...
        return { ok: false, governance: finalizedGovernance, task: executionTasks.get(task.id) ?? task };
    }
    const completedTask = {
        ...task,
        status: "completed",
    ...
    createExecutionReceipt(mission.id, completedTask.id, completedTask.assignedAgentId, options?.summary ?? `${completedTask.title} completed successfully.`, "success", options?.evidenceRefs ?? [completedTask.id, mission.id], [
    ...
    if (updatedMission && (updatedMission.status === "active" || updatedMission.status === "planned")) {
        ...
        const newlyReleasedTasks = queuedTasks.filter((candidate) => candidate.coordination?.releaseReady && !queuedTaskReleaseState.get(candidate.id));
    ...
    return { ok: true, governance: finalizedGovernance, task: completedTask, dispatchedTask };
};
const deriveMissionStatusFromTasks = (mission, tasks = missionTasks(mission.id)) => {
    if (mission.status === "halted")
        return "halted";
    if (mission.status === "completed" && tasks.length === 0)
        return "completed";
    ...
    if (blockedTaskCount > 0)
        return "blocked";
    if (tasks.length > 0 && queuedTaskCount === 0 && runningTaskCount === 0)
        return "completed";
    if (mission.status === "planned" && tasks.length === 0)
        return "planned";
    return "active";
};
const deriveMissionStepsFromTasks = (mission, tasks = missionTasks(mission.id)) => {
    ...
    const summarizePhase = (phaseTasks, fallbackPending = "pending") => {
        if (phaseTasks.length === 0)
            return fallbackPending;
        if (phaseTasks.some((task) => task.status === "blocked" || task.status === "cancelled"))
            return "blocked";
        if (phaseTasks.every((task) => task.status === "completed"))
            return "completed";
        if (phaseTasks.some((task) => task.status === "running" || task.status === "completed"))
            return "in_progress";
        return fallbackPending;
    };
    const prepareStatus = summarizePhase(prepareTasks, "in_progress");
    const executeStatus = prepareTasks.length > 0 && !prepareTasks.every((task) => task.status === "completed")
        ? "pending"
        : summarizePhase(executeTasks, mission.status === "planned" ? "pending" : "in_progress");
    const auditStatus = executeTasks.length > 0 && !executeTasks.every((task) => task.status === "completed")
        ? "pending"
    ...
    return mission.steps.map((step, index) => {
        ...
        if (index === 1)
            return { ...step, status: prepareStatus };
        if (index === 2) {
            const governanceStatus = auditStatus === "completed" ? "completed" : auditStatus === "in_progress" ? "in_progress" : "pending";
            return { ...step, status: governanceStatus };
        }
        if (index === 3) {
            const executionStatus = executeStatus === "completed" && auditStatus === "completed"
                ? "completed"
...
const syncMissionDerivedState = (missionId) => {
    ...
    for (const task of tasks) {
        ...
        if (!coordination)
            continue;
        const nextReadyAt = dependencyState.satisfied && task.status === "queued"
            ? coordination.readyAt ?? now()
            : undefined;
        const nextBlockedByTaskIds = dependencyState.blockedByTaskIds.length > 0
    ...
    if (workspace) {
        const nextWorkspaceState = nextStatus === "completed" ? "sealed" : nextStatus === "halted" || nextStatus === "blocked" ? "paused" : "active";
...
const evaluateTaskSelectionCandidate = (task, requestedAgentId, selectionContext) => {
    ...
    if (workspaceState === "paused" || workspaceState === "sealed")
        return undefined;
    ...
    if (governanceStatus === "blocked")
        return undefined;
    const dependencySatisfied = task.coordination?.releaseReady ?? taskDependencyState(task).satisfied;
...
const compareTaskSelectionCandidates = (left, right) => {
    const missionStatusWeight = (status) => (status === "active" ? 0 : 1);
    const workspaceStateWeight = (state) => (state === "active" ? 0 : 1);
    return (missionStatusWeight(left.missionStatus) - missionStatusWeight(right.missionStatus) ||
...
const selectNextQueuedTask = (requestedAgentId, missionId, selector) => {
...
const explainTaskIneligibility = (task, requestedAgentId) => {
    const reasons = [];
    if (task.status !== "queued")
        reasons.push(`task status ${task.status} is not claimable`);
    if (requestedAgentId && task.assignedAgentId !== requestedAgentId) {
    ...
    const governanceStatus = task.governance ? normalizeGovernanceDecision(task.governance).status : undefined;
    if (governanceStatus === "blocked")
        reasons.push("task already carries a blocked governance decision");
...
const reconcileRecoveredState = async () => {
    ...
    const revokedLeaseIds = [];
    for (const task of [...executionTasks.values()]) {
        if (task.status === "running") {
            const continuityRecovery = task.governance?.route?.continuity === "degraded" || task.governance?.route?.continuity === "disconnected"
                ? {
                    continuity: task.governance.route.continuity,
                    delegatedAuthorityAnchor: task.governance.route.delegatedAuthorityAnchor,
                    continuityReasoning: task.governance.route.continuityReasoning,
        ...
        if (task.status === "completed" && !hasReceiptForTask(task.id, "success")) {
            createExecutionReceipt(task.missionId, task.id, task.assignedAgentId, `${task.title} recovered as completed from persisted state.`, "success", [task.id, task.missionId], task.governance
                ? [
    ...
    for (const mission of [...missions.values()]) {
        ...
        const workspace = findMissionWorkspace(mission.id);
        if (workspace) {
            const nextWorkspaceState = missionStatus === "completed" ? "sealed" : missionStatus === "halted" || missionStatus === "blocked" ? "paused" : "active";
    ...
    const runningTaskAgents = new Set([...executionTasks.values()].filter((task) => task.status === "running").map((task) => task.assignedAgentId));
        ...
        for (const missionId of touchedMissionIds) {
            await commitLedgerEvent(missionId, "agent-os.runtime.reconciled", {
                ...
                continuityRecoveredTaskIds: requeuedTaskIds.filter((taskId) => {
                    const task = executionTasks.get(taskId);
                    return Boolean(task?.missionId === missionId &&
                        task.output?.continuityRecovery);
                }),
...
const reconcileStaleRunningTasks = async () => {
    ...
    for (const task of [...executionTasks.values()]) {
        ...
        if (Number.isNaN(heartbeatAt) || timestamp - heartbeatAt < heartbeatTimeoutMs)
            continue;
        await queueTaskRetry(task, "agent-os", `Worker heartbeat exceeded timeout window of ${Math.round(heartbeatTimeoutMs / 1000)}s.`);
    }
};
const autonomousSafeTools = new Set(["ledger", "policy", "witness", "simulation", "planning", "docs"]);
const isAutonomouslyDispatchableTask = (task) => task.status === "queued" &&
...
const isAutonomouslyCompletableTask = (task) => task.status === "running" &&
    (task.ownerRole === "auditor" || task.ownerRole === "planner" || task.coordination?.phase === "audit") &&
    task.requiredTools.every((toolId) => autonomousSafeTools.has(toolId));
const runAutonomyTick = async () => {
    if (autonomyLoopRunning)
        return;
    ...
    try {
        ...
        for (const mission of [...missions.values()]) {
            ...
            if (!runningTask) {
                ...
                if (!anyRunningTask) {
                    const dispatchedTask = await dispatchNextEligibleTask(mission, "Dispatched by autonomous governed continuity loop.", isAutonomouslyDispatchableTask);
...
const seedMissionExecutionTasks = async (mission) => {
    const existing = missionTasks(mission.id);
    if (existing.length > 0)
        return existing;
    const contextTask = createExecutionTask(mission, "Assemble workspace context bundle", "executor", ["editor", "ledger"], { objective: mission.objective, workspaceId: mission.workspaceId }, {
        phase: "prepare",
        dependsOnTaskIds: [],
        releaseCondition: "workspace context required before implementation"
    });
    const implementationTask = createExecutionTask(mission, "Run governed implementation pass", "executor", mission.requiredTools, { targetSystem: mission.targetSystem, successMetrics: mission.successMetrics }, {
        phase: "execute",
    ...
    const auditTask = createExecutionTask(mission, "Audit evidence and release recommendation", "auditor", ["ledger", "policy"], { governanceProfile: mission.governanceProfile }, {
        phase: "audit",
...
const progressExecutionLoop = async (mission, action) => {
    ...
    if (action === "halt") {
        ...
        for (const task of missionTasks(mission.id)) {
            if (task.status === "queued" || task.status === "running") {
                ...
                setAgentRuntimeStatus(task.assignedAgentId, "ready");
                createExecutionReceipt(mission.id, task.id, task.assignedAgentId, `${task.title} halted by operator action.`, "halted", [task.id, mission.id]);
    ...
    if (action === "complete") {
        for (const task of missionTasks(mission.id)) {
            if (task.status === "queued") {
                ...
                }, task.assignedAgentId, "Mission completion sweep.");
                await completeTaskWithGovernance(mission, runningTask, {
...
app.post("/tasks/:taskId/claim", async (req, res) => {
    ...
    const claimedTask = claimTaskForAgent({
        ...
        governance
    }, agentId, req.body.statusNote ?? "Claimed by worker runtime.");
    prewarmTaskGovernanceArtifacts(mission, claimedTask, "completion");
    ensureMissionMemory(mission.id).push({
        id: id("mem"),
        missionId: mission.id,
...
app.post("/tasks/:taskId/actions", async (req, res) => {
    ...
    const action = {
        ...
        constraints: missionToolLeases(mission.id).find((lease) => lease.toolId === toolId)?.constraints ?? ["no explicit constraints recorded"],
        governance,
        createdAt: timestamp,
        updatedAt: timestamp
    };
...
app.post("/tasks/:taskId/actions/:actionId/execute", async (req, res) => {
    ...
    if (!action || action.taskId !== task.id)
        return res.status(404).json({ error: "tool_action_not_found" });
    if (action.status !== "approved")
        return res.status(409).json({ error: "tool_action_not_approved", action });
    const mission = missions.get(task.missionId);
    if (!mission)
        return res.status(404).json({ error: "mission_not_found" });
...
app.post("/leases/:leaseId/renew", async (req, res) => {
    ...
    const renewed = await renewMissionLeases(lease.missionId, req.body.actor ?? lease.agentId, req.body.reason ?? `manual lease renewal for ${lease.toolId}`, 365 * 24 * 60 * 60 * 1000, Number(req.body.extensionMs ?? leaseExtensionMs));
    await schedulePersist();
    res.json({ renewed });
});
...
app.post("/missions", async (req, res) => {
    ...
    const assignedAgents = req.body.assignedAgents ?? ["agent-planner", "agent-executor", "agent-auditor"];
    const requiredTools = req.body.requiredTools ?? ["shell", "editor", "ledger"];
...
app.post("/missions/:missionId/advance", async (req, res) => {
    ...
    res.json({
        ...
        suggestedNextActions: updatedMission.status === "completed"
