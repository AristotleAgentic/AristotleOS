...
export function createChainClient(config: ChainClientConfig): ChainClient {
  ...
  const missionIds = new Map<string, MissionChainIds>();
  ...
  const getJson = async (path: string): Promise<any> => (await fetch(`${kernelBase}${path}`)).json();

  const ensureConstitution = async () => {
    if (constitutionEnsured) return;
    await postJson("/v2/meta-authority-envelope", maeBody(keyId));
    constitutionEnsured = true;
  };

  const ensureMissionChain = async (mission: OperatingMission): Promise<MissionChainIds> => {
    await ensureConstitution();
    const cached = missionIds.get(mission.id);
    if (cached) return cached;
    if (!gateId) gateId = (await getJson("/v2/commit-gate"))?.commit_gate_id;
    ...
    const ids: MissionChainIds = { maeId: MAE_ID, wardId, envelopeId, gateId: gateId ?? "" };
  ...
  const runCommit = async (input: CommitTaskInput, ids: MissionChainIds): Promise<any> => {
    ...
    if (!warrant?.warrant_id) return { warrantFailed: true, warrant };
...
function wardBody(mission: OperatingMission, wardId: string) {
  return {
    ward_id: wardId,
    mae_id: MAE_ID,
...
function envelopeBody(mission: OperatingMission, wardId: string, envelopeId: string) {
  return {
