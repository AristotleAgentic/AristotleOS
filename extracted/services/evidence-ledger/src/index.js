...
import { appendGelRecord, loadGelChain, verifyGelChain, exportEvidenceBundle } from "@aristotle/execution-control-runtime";
...
const ledgerSigningPrivateKey = ledgerSigningPrivateKeyPath
    ...
    : null;
const ledgerSigningPublicKey = resolvedLedgerPublicKeyPath && existsSync(resolvedLedgerPublicKeyPath)
...
const isIndexedArtifact = (value) => {
    if (!isRecord(value))
        return false;
    return typeof value.id === "string" && typeof value.artifactType === "string" && artifactTypes.has(value.artifactType);
};
...
const readString = (value) => (typeof value === "string" && value.length > 0 ? value : undefined);
const readStringArray = (value) => Array.isArray(value) ? value.filter((item) => typeof item === "string" && item.length > 0) : [];
...
const verifyArtifact = (artifact) => {
    ...
    if (ledgerSigningPrivateKey && ledgerSigningPublicKey) {
        ...
        const verified = cryptoVerify(null, Buffer.from(digest, "utf8"), ledgerSigningPublicKey, Buffer.from(signature, "base64"));
...
const eventPhase = (event) => {
    if (/tool-action/.test(event.eventKind))
        return "tool-action";
    if (/completed|finality|witness/.test(event.eventKind))
        return "completion";
    return "dispatch";
};
const synthesizeArtifactsFromEvent = (event) => {
    ...
    const envelopeId = readString(payload.envelopeId) ??
    ...
    const witnessStatus = readString(payload.witnessStatus) === "satisfied" ||
        readString(payload.witnessStatus) === "unsatisfied" ||
        readString(payload.witnessStatus) === "not-required"
        ? readString(payload.witnessStatus)
        : "not-required";
    const artifacts = [];
    if (envelopeId) {
        artifacts.push({
    ...
    if (decisionId) {
        artifacts.push({
            ...
            targetId,
            decision: readString(payload.decision) === "deny" || readString(payload.decision) === "halt"
                ...
                : readString(payload.projectedOutcome) === "halt"
                    ...
                    : /blocked|halted/.test(event.eventKind)
                        ? "deny"
                        : "allow",
            reasons: reasons.length > 0 ? reasons : readString(projectedRoute.failoverReasoning) ? [readString(projectedRoute.failoverReasoning)] : [],
            killSwitchState: readString(payload.killSwitchState) === "active"
                ? "active"
    ...
    if (event.eventKind === "governance.kill-switch.updated" &&
    ...
    if (event.eventKind === "counterfactual.projected" &&
        ...
        artifacts.push({
            ...
            chainId: event.chainId,
            state: "active",
            reason: readString(projectedRoute.failoverReasoning) ??
                `Counterfactual ${readString(scenario.scope) ?? "global"} halt projection.`,
            scope: readString(scenario.scope),
    ...
    if (agentId && agentFingerprint) {
        artifacts.push({
            ...
            status: readString(payload.agentVerificationStatus) === "degraded" || readString(payload.agentVerificationStatus) === "revoked"
                ? readString(payload.agentVerificationStatus)
                : "verified",
            attributes: {
    ...
    if (deviceId && deviceFingerprint) {
        artifacts.push({
            ...
            issuerRef: readString(payload.issuer) ?? "mission.command",
            status: readString(payload.deviceVerificationStatus) === "degraded" || readString(payload.deviceVerificationStatus) === "revoked"
                ? readString(payload.deviceVerificationStatus)
                : "verified",
            attributes: {
    ...
    if (event.eventKind === "agent-os.execution.task.autonomous-completed") {
        artifacts.push({
            ...
            continuity: readString(payload.continuity) === "degraded" || readString(payload.continuity) === "disconnected"
                ? readString(payload.continuity)
                : "stable",
    ...
    if (event.eventKind === "agent-os.runtime.reconciled" && Array.isArray(payload.continuityRecoveredTaskIds)) {
        for (const [index, recoveredTaskId] of payload.continuityRecoveredTaskIds.entries()) {
            if (typeof recoveredTaskId !== "string")
                continue;
            artifacts.push({
    ...
    if (event.eventKind === "assurance.report.attested") {
        ...
        artifacts.push({
            ...
            systemPosture: readString(report.systemPosture) === "insurable" || readString(report.systemPosture) === "halted"
                ? readString(report.systemPosture)
                : "conditional",
            assurancePosture: reportScope === "mission" &&
                ...
                ? readString(mission.assurancePosture)
                : undefined,
...
const eventMatchesRelatedId = (event, relatedId) => {
    if (!relatedId)
        return true;
...
const extractArtifactsFromEvent = (event) => {
...
const ingestArtifactsFromPayload = (event) => {
    for (const artifact of extractArtifactsFromEvent(event)) {
        indexedArtifacts.set(artifact.id, artifact);
    }
};
const artifactTimeline = (traceId, artifactType, relatedId, branchId) => {
    ...
    const items = sourceItems.flatMap((event) => extractArtifactsFromEvent(event).filter((artifact) => (artifactType ? artifact.artifactType === artifactType : true)));
    const seen = new Set();
...
await loadState();
app.get("/health", (_req, res) => res.json({ ok: true, service: "evidence-ledger", persistedStatePath: statePath, committedEvents: committed.length }));
...
app.get("/replay", (req, res) => {
    ...
    if (branchId && typeof branchId === "string") {
        const items = (hypothetical.get(branchId) ?? []).filter((event) => eventMatchesRelatedId(event, typeof relatedId === "string" ? relatedId : undefined));
        return res.json({ committed: false, items });
    }
    const items = committed.filter((event) => (typeof traceId === "string" ? event.traceId === traceId : true) &&
...
app.get("/timeline", (req, res) => {
    ...
    res.json({
        committed: committed.filter((event) => (traceId ? event.traceId === traceId : true) && eventMatchesRelatedId(event, relatedId)),
        branches: [...branches.values()]
    });
});
app.get("/artifacts", (req, res) => {
    ...
    const relatedId = typeof req.query.relatedId === "string" ? req.query.relatedId : undefined;
    const artifactType = typeof req.query.artifactType === "string" && artifactTypes.has(req.query.artifactType)
...
// governance decision.
// ---------------------------------------------------------------------------
const gelPath = resolve(process.cwd(), process.env.EVIDENCE_LEDGER_GEL_PATH ?? "./data/evidence-ledger.gel.jsonl");
app.post("/gel/append", async (req, res) => {
    const { ward, action, decision, warrant, actor } = req.body;
...
app.post("/gel/export", (req, res) => {
    const { ward, recordId, authorityEnvelope, warrant } = req.body;
    if (!ward) {
        return res.status(400).json({
