...
const isTruthy = (value?: string) => value === "1" || value === "true" || value === "TRUE";

export const runGatewayPreflight = (): GatewayPreflightResult => {
  const mode = process.env.NODE_ENV === "production" ? "production" : "development";
  const checks: GatewayPreflightResult["checks"] = [];
  ...
  checks.push({
    ...
    status:
      operatorSessionEnforcement && !operatorSessionSecret
        ? mode === "production"
          ? "fail"
          : "warn"
  ...
  checks.push({
    ...
    detail:
      mode === "production" && serviceDiscoveryMode === "local"
  ...
  checks.push({
    ...
    detail:
      evidenceLedgerStatePath && agentOsStatePath
  ...
  if (failedChecks.length > 0 && allowInsecureProduction) {
    return {
      ...
      checks: [
        ...checks.map((check) =>
          check.status === "fail"
            ? { ...check, status: "warn" as const, detail: `${check.detail} Override active via ALLOW_INSECURE_PRODUCTION_BOOT.` }
            : check
        )
      ]
    };
