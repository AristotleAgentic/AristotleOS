...
const readinessCriticalServices = new Set(
  (process.env.GATEWAY_CRITICAL_SERVICES ?? "governance-kernel,policy-compiler,evidence-ledger,meta-authority-registry,simulation-engine,witness-service,execution-gate,agent-os")
    .split(",")
...
type UpstreamProbe = {
  ...
  activeKillScopes?: unknown[];
  error?: string;
};

const roundMs = (value: number) => Math.round(value * 1000) / 1000;

const probeService = async (service: (typeof observedServices)[number], timeoutMs = readinessTimeoutMs): Promise<UpstreamProbe> => {
...
const evaluateReadiness = async () => {
  ...
  const activeGovernanceHalt = services.some(
    (service) => (service.name === "governance-kernel" || service.name === "execution-gate") && service.killSwitchState === "active"
  );
...
const prometheusEscape = (value: string) => value.replace(/\\/g, "\\\\").replace(/"/g, '\\"').replace(/\n/g, "\\n");

const renderGatewayMetrics = (readiness: Awaited<ReturnType<typeof evaluateReadiness>>) => {
  const lines: string[] = [
  ...
  for (const service of readiness.services) {
    lines.push(
      `aristotle_upstream_ready{service="${prometheusEscape(service.name)}",critical="${service.critical ? "true" : "false"}"} ${service.ok ? 1 : 0}`
    );
  }
...
const hrTimeUnixNano = () => String(BigInt(Date.now()) * 1_000_000n);
const parseOtelResourceAttributes = () =>
  Object.fromEntries(
...
const emitOtelSpan = (span: {
  ...
  if (otelTracesExporter !== "otlp" || !otelExporterEndpoint) return;
...
const appendTrialRecord = (evaluation: TrialEvaluation) => {
  ...
  if (trialGelRecords.length > 100) trialGelRecords.pop();
};

const evaluateTrialRequest = (body: Record<string, unknown>, approval?: "approve" | "deny" | "more_info" | "reduced_authority") => {
