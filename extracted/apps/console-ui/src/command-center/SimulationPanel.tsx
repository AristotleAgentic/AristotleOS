...
import { Badge, Panel, cx, decisionTone } from "./primitives.js";
...
export function SimulationPanel() {
  ...
  return (
    <div className="ac-grid" style={{ gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1.2fr)", alignItems: "start" }}>
      <Panel title="Counterfactual Console" icon={<FlaskConical size={15} />} right={<Badge tone="cyan">no live effect</Badge>}>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <button className="ac-btn is-primary" style={{ justifyContent: "center", marginTop: 4 }} onClick={() => setOutcome(scenario.run())}>
            <Play size={14} /> Run simulation
          </button>
        </div>
      </Panel>

      <Panel title="Simulated Outcome" icon={<FlaskConical size={15} />}>
        {outcome ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div className="ac-confirm-warn" style={{ background: "rgba(56,212,232,0.06)", borderColor: "var(--ac-cyan-dim)", color: "var(--ac-text-2)" }}>
              <FlaskConical size={15} color="var(--ac-cyan)" /> Counterfactual only. No warrant issued, no evidence written, no live state changed.
            </div>
          </div>
        ) : (
          <div className="ac-empty">Choose a scenario and run the simulation to see the governed outcome.</div>
        )}
      </Panel>
    </div>
  );
