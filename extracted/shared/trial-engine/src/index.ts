...
const block = (source: string, name: string) => source.match(new RegExp(`${name}\\s+"?([^"{\\s]+)?"?\\s*\\{([\\s\\S]*?)\\}`, "m"));
const stringField = (body: string, key: string, fallback = "") => body.match(new RegExp(`${key}\\s*=\\s*"([^"]+)"`))?.[1] ?? fallback;
const boolField = (body: string, key: string, fallback = false) => {
  const value = body.match(new RegExp(`${key}\\s*=\\s*(true|false)`))?.[1];
  return value ? value === "true" : fallback;
};
const numberField = (body: string, key: string) => {
  const value = body.match(new RegExp(`${key}\\s*=\\s*([0-9]+(?:\\.[0-9]+)?)`))?.[1];
  return value ? Number(value) : undefined;
};
const stringArrayField = (body: string, key: string) => {
...
export function parseGovernanceSource(source: string): TrialGovernancePolicy {
  ...
  const policy: TrialGovernancePolicy = {
    source,
...
export function validateGovernanceSource(source: string) {
  const errors: Array<{ path: string; message: string }> = [];
  let policy: TrialGovernancePolicy | undefined;
  try {
  ...
  if (policy) {
    if (!policy.ward.sovereign) errors.push({ path: `ward.${policy.ward.id}.sovereign`, message: "ward sovereign is required" });
    if (policy.authority.ward !== policy.ward.id) errors.push({ path: `authority_envelope.${policy.authority.id}.ward`, message: "authority envelope must bind to an existing ward" });
    if (policy.commitGate.requireAuthority !== policy.authority.id) errors.push({ path: `commit_gate.${policy.commitGate.id}.require_authority`, message: "commit gate must reference an existing authority envelope" });
    if (policy.commitGate.requireWarrant && policy.warrantPolicy.standingPower) errors.push({ path: `warrant_policy.${policy.warrantPolicy.id}.standing_power`, message: "warrant policy must not grant standing machine power" });
    if (!policy.gel.hashChain) errors.push({ path: "gel.hash_chain", message: "GEL hash_chain must remain enabled for replayable evidence" });
  }
  return { ok: errors.length === 0, errors, policy };
}

const addStep = (steps: TrialPipelineStep[], id: string, label: string, status: TrialPipelineStep["status"], detail: string) => {
  steps.push({ id, label, status, detail });
};

const plusSeconds = (iso: string, seconds: number) => new Date(new Date(iso).getTime() + seconds * 1000).toISOString();
const secondsFromDuration = (duration: string) => Number(duration.match(/([0-9]+)\s*s/)?.[1] ?? 60);
...
export function evaluateTrialAction(input: {
  ...
  const steps: TrialPipelineStep[] = [];
  let decision: TrialDecisionState = "PERMIT";
  ...
  const gelRecord: TrialGelRecord = {
...
export function planGovernanceChange(source: string, currentSource = PAYMENTS_GOVERNANCE_SOURCE) {
  const next = validateGovernanceSource(source);
  const current = validateGovernanceSource(currentSource);
  if (!next.policy) return { ok: false, errors: next.errors, changes: [] as string[], nextPolicyHash: undefined };
  const changes: string[] = [];
  if (current.policy?.policyHash !== next.policy.policyHash) changes.push(`policy hash changes ${current.policy?.policyHash ?? "none"} -> ${next.policy.policyHash}`);
  if (current.policy?.authority.maxAmount !== next.policy.authority.maxAmount) changes.push(`authority max_amount changes ${current.policy?.authority.maxAmount ?? "none"} -> ${next.policy.authority.maxAmount ?? "none"}`);
  if (current.policy?.commitGate.deferAboveOrEqual !== next.policy.commitGate.deferAboveOrEqual) changes.push(`defer threshold changes ${current.policy?.commitGate.deferAboveOrEqual ?? "none"} -> ${next.policy.commitGate.deferAboveOrEqual ?? "none"}`);
  if (current.policy?.authority.revoked !== next.policy.authority.revoked) changes.push(`revocation changes ${current.policy?.authority.revoked ?? false} -> ${next.policy.authority.revoked ?? false}`);
