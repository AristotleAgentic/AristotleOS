...
import { HmacKeyring, type Keyring } from "./hash.js";
import { isoPlusSeconds, newId, nowIso } from "./ids.js";
import { InMemoryGovernanceStore, type GovernanceStore } from "./store.js";
import {
  appointGovernor,
  commitRequestFor,
  constituteWard,
...
function world(): { store: GovernanceStore; keyring: Keyring; keyId: string } {
...
function originAct(actor: string, actorKind: "human" | "institution", method: Parameters<typeof constituteWard>[3]["human_origin_act"]["method"]) {
...
export function buildPayments(): ScenarioWorld {
  ...
  const propose = (opts: ProposeOptions = {}): ProposeResult => {
...
export function buildDrone(): ScenarioWorld {
  const { store, keyring, keyId } = world();
...
export function buildHealthcare(): ScenarioWorld {
  const { store, keyring, keyId } = world();
...
// ---------------------------------------------------------------------------
// D. Federation — Field drone operator (Ward A) and Emergency response (Ward B)
// ---------------------------------------------------------------------------
...
export interface FederationWorld {
  ...
  propose: (opts?: ProposeOptions & { withAgreement?: boolean }) => ProposeResult;
}

/** Build a cross-domain federation world. `trusted=false` removes Ward B's MAE from Ward A's trust list. */
export function buildFederation(trusted = true): FederationWorld {
  const { store, keyring, keyId } = world();
  ...
  const propose = (opts: ProposeOptions & { withAgreement?: boolean } = {}): ProposeResult => {
    const action: ProposedAction = {
      proposed_action_id: newId("act"),
