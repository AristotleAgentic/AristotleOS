 ...
 */
import { computeGelRecordHash, signGelRecord, verifyGelRecordSignatures } from "./hash.js";
 ...
 */
export function finalizeAndAppend(store, keyring, keyId, draft) {
    const sequence = store.gelLength();
    const previous_gel_hash = store.gelHeadHash();
    const base = { ...draft, sequence, previous_gel_hash };
    const gel_record_hash = computeGelRecordHash(base);
    const record = { ...base, gel_record_hash, signatures: [] };
    record.signatures = [signGelRecord(keyring, keyId, record)];
 ...
 */
export function verifyGelChain(records, keyring) {
    const results = [];
    for (let i = 0; i < records.length; i++) {
        ...
        const recomputed = computeGelRecordHash(r);
        ...
        if (keyring && !verifyGelRecordSignatures(keyring, r)) {
 ...
 */
export function verifyGelRecords(records, keyring) {
    const results = [];
    for (let i = 0; i < records.length; i++) {
        const r = records[i];
        if (computeGelRecordHash(r) !== r.gel_record_hash) {
            results.push(fromViolations([violation("gel-tamper-evident", `record ${i} content hash mismatch (tampered?)`)]));
        }
        if (keyring && !verifyGelRecordSignatures(keyring, r)) {
            results.push(fromViolations([violation("gel-signature", `record ${i} signature invalid`)]));
 ...
 */
export function assertAuthorityChainComplete(record) {
    const v = [];
    const need = [
    ...
    for (const [field, inv] of need) {
        if (!record[field])
            v.push(violation(inv, `allowed GEL record missing ${String(field)}`));
    }
