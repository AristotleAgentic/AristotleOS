...
export function maeIdsInScope(snapshot: StoreSnapshot, filter?: ScopeFilter): Set<string> | null {
  if (!filter || (!filter.maeId && !filter.tenantId)) return null;
  const ids = new Set<string>();
  for (const mae of snapshot.maes) {
    if (filter.maeId && mae.mae_id === filter.maeId) ids.add(mae.mae_id);
    if (filter.tenantId && mae.tenant_id === filter.tenantId) ids.add(mae.mae_id);
  }
  return ids;
}
...
export function scopeSnapshot(snapshot: StoreSnapshot, filter?: ScopeFilter): StoreSnapshot {
...
export function tenantSummaries(snapshot: StoreSnapshot): TenantSummary[] {
  const byTenant = new Map<string, TenantSummary>();
  const tenantOfMae = new Map<string, string>();
  for (const mae of snapshot.maes) {
  ...
  const bump = (maeId: string | undefined, key: "wards" | "authority_envelopes" | "warrants" | "gel_records") => {
    if (!maeId) return;
  ...
  for (const w of snapshot.wards) bump(w.mae_id, "wards");
  for (const e of snapshot.envelopes) bump(e.mae_id, "authority_envelopes");
  for (const w of snapshot.warrants) bump(w.mae_id, "warrants");
  for (const r of snapshot.gel) bump(r.mae_id, "gel_records");
