...
import { fromViolations, valid, violation, type ValidationResult, type Violation } from "./errors.js";
import { finalizeAndAppend, type GelDraft } from "./gel.js";
import { newId } from "./ids.js";
import { context, maeIsLive } from "./validators.js";
import { evaluateCommit, type CommitOptions } from "./commit-gate.js";
import type { GovernanceStore } from "./store.js";
import type { ChainRefs, CommitDecision, CommitDecisionKind, CommitRequest, FederationAgreement } from "./types.js";
...
export function validateFederation(
  ...
  if (!localMae) return fromViolations([violation("federation-local-mae-missing", agreement.local_mae_id)]);
  if (!foreignMae) return fromViolations([violation("federation-foreign-mae-missing", agreement.foreign_mae_id)]);

  if (!maeIsLive(localMae, now)) v.push(violation("federation-local-mae-invalid", "local MAE not currently valid"));
  if (!maeIsLive(foreignMae, now)) v.push(violation("federation-foreign-mae-invalid", "foreign MAE not currently valid"));

  // The local constitution must both permit federation and recognise the foreign MAE.
  if (!localMae.federation_rules.federation_allowed) v.push(violation("federation-not-allowed", "local MAE forbids federation"));
  ...
  if (agreement.revoked_at) v.push(violation("ward-to-ward-trust-required", "federation agreement revoked"));
 ...
 */
export function evaluateFederatedCommit(store: GovernanceStore, request: CommitRequest, opts: CommitOptions): CommitDecision {
