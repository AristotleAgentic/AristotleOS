...
export function revokeWarrant(store, warrantId, at, out = empty()) {
...
export function revokeEnvelope(store, envelopeId, at, out = empty()) {
    ...
    for (const w of store.warrantsForEnvelope(envelopeId))
        revokeWarrant(store, w.warrant_id, at, out);
    return out;
}
/** Revoke a Ward and cascade to its Envelopes and Warrants. */
export function revokeWard(store, wardId, at, out = empty()) {
    ...
    for (const env of store.envelopesForWard(wardId))
        revokeEnvelope(store, env.authority_envelope_id, at, out);
    // Catch any warrants bound to the ward directly (defensive).
    for (const w of store.warrantsForWard(wardId))
        revokeWarrant(store, w.warrant_id, at, out);
    return out;
}
/** Revoke a Meta Authority Envelope and cascade to every dependent Ward. */
export function revokeMae(store, maeId, at, out = empty()) {
    ...
    for (const ward of store.wardsForMae(maeId))
        revokeWard(store, ward.ward_id, at, out);
    return out;
}
/** Suspend a Ward (reversible) without cascading hard revocation. */
export function suspendWard(store, wardId, at) {
    const ward = store.getWard(wardId);
    if (!ward)
        return;
