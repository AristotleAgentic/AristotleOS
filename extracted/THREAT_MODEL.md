...
| F6 | Malicious package dependency (supply-chain attack) | Repository uses `corepack` pinning of pnpm@10.32.1. Dependency license review and SBOM generation are documented in `RELEASE_CHECKLIST.md`. | None directly testable. | A compromised dependency could subvert any layer. | SBOM publication; signed releases; npm provenance (`npm publish --provenance`). |
